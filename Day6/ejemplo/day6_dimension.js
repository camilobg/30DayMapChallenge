require(
    ...
    (Map, SceneView, GeoJSONLayer, GraphicsLayer, Graphic, Polyline) => {
        ...
        const START_LON = -22.6056;
        const START_LAT = 63.985;
        const START_Z   = 2000;

        // Camera view
        const CAMERA_HEADING = 0;      // looking north
        const CAMERA_TILT    = 60;     // degrees
        const CAMERA_ZOOM    = 9;      // zoom level
        const CAMERA_SPEED_FACTOR = 0.5; // how smooth the camera follows (higher = slower)

        // Animation
        const CAR_SPEED_INDICES_PER_SECOND = 500; // how fast to move along route
